...
const selectVisibleCrystals = (state) => {
  ...
  return memoizedConvertVisibleCrystals(
      matrix,
      markedCrystalIds,
  );
};
...
const selectMarkedCrystals = (state) => {
  ...
  return memoizedConvertMarkedCrystals(
      matrix,
      markedCrystalIds,
      linkOriginId,
  );
...
const memoizedSlice = memoizeOne(
    (matrix) => matrix.slice(BUFFER_ZONE_ROW_COUNT)
);
...
const ENTER_MATRIX_INDEX = createMatrixIndex(
    BUFFER_ZONE_ROW_COUNT,
    MIDDLE_COLUMN_INDEX,
);
