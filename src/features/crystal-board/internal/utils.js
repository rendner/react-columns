...
export const convertXToColumnIndex = (xPosition) => {
  return Math.min(
      crystalBoardDomainConstants.COLUMNS_PER_ROW - 1,
      Math.floor(xPosition / crystalConstants.CELL_SIZE),
  );
};

export const convertYToRowIndex = (yPosition) => {
  return Math.min(
      crystalBoardDomainConstants.ROW_COUNT - 1,
      Math.floor(yPosition / crystalConstants.CELL_SIZE),
  );
};
